...
| 2026-08-05 | Claude Code (Katchi's PC) | **Tier 1 + Tier 2 from `COGPRINT_TIER1_TIER2_EXECUTION_PROMPT.md`** — the launch blockers and the instrumentation a beta needs. Magic-link email auth (additive to recovery tokens; unverified addresses never accepted, non-enumerating request endpoint); GDPR export + erasure; first-party analytics with a D1/D7 funnel; email review reminders (the channel that finally acts on the forgetting curve); onboarding sample + first-round copy. Deploy config updated for all new env vars. **Also fixed a real bug found by the smoke test, not the suite:** a pre-Alembic database was being stamped at `head`, asserting a currency it did not have, so a pulled schema change crashed at the first query — now stamped at the baseline revision and migrated forward, with a regression test. 133 tests green. | ✅ done |
