...
needs a key), **and the consumer app (`app/`)** — all 5 routes (Paste/Plan/Cards/Grow/Checks),
real/sham `InsightProvider` split, Pomodoro timer, PWA icons — verified end-to-end against a
live backend as of 2026-06-18/28. **Not built / not started:** any deployment (backend,
consumer app, or research frontend all run locally only — no hosting, no domain), real
user testing (everything tested so far is the owner with seeded/synthetic data), Play
Store / App Store packaging.
